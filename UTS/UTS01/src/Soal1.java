...
class DLinkedList{
    ...
    void insertLast(char data){
		...
		*/
        if (mid==null && last==null) {      // insert 1st element
            mid = new DNode(data);
            last = mid; header.next = mid;
            size++;
        } else if (last != null && last.next == null && mid != null && mid.next== null) { // 2nd element
            last = new DNode(data,mid,null);
            mid.next = last; header.next = mid;
            size++;
        } else if (size%2==0) { // genap, mid tetap
            ...
            last.next = null;
            size++;
        } else if (size%2==1) { // ganjil, mid geser 1 ke kanan
            DNode newNode = new DNode(data);
            last.next = newNode;
            newNode.prev = last;
            last = newNode;
